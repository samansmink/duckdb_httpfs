...
EXTENSION_FLAGS=\
-DDUCKDB_EXTENSION_NAMES="httpfs" \
...
#### Add more of the DuckDB in-tree extensions
DUCKDB_EXTENSION_CONFIGS?=
ifneq ("${DUCKDB_EXTENSION_CONFIGS}", "")
	EXTENSION_CONFIG_FLAG:=-DDUCKDB_EXTENSION_CONFIGS=${DUCKDB_EXTENSION_CONFIGS}
endif

BUILD_FLAGS=-DEXTENSION_STATIC_BUILD=1 $(EXTENSION_FLAGS) ${EXTENSION_CONFIG_FLAG} ${EXTRA_EXTENSIONS_FLAG} $(OSX_BUILD_FLAG) $(TOOLCHAIN_FLAGS) -DDUCKDB_EXPLICIT_PLATFORM='${DUCKDB_PLATFORM}'
CLIENT_FLAGS:=

#### Main build
# For regular CLI build, we link the httpfs extension directly into the DuckDB executable
