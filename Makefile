...
ifneq ("${DUCKDB_EXTENSION_CONFIGS}", "")
	EXTENSION_CONFIG_FLAG:=-DDUCKDB_EXTENSION_CONFIGS=$(PROJ_DIR)/${DUCKDB_EXTENSION_CONFIGS}
endif

BUILD_FLAGS=-DEXTENSION_STATIC_BUILD=1 $(EXTENSION_FLAGS) ${EXTENSION_CONFIG_FLAG} ${EXTRA_EXTENSIONS_FLAG} $(OSX_BUILD_FLAG) $(TOOLCHAIN_FLAGS)
CLIENT_FLAGS:=
